  ...
  submitForm() {
    if (this.taskform.valid && this.userService.userData._id) {
      if (this.isformTypeUpdate) {
        this.taskService.updateTask({ ...this.taskform.value, _id: this.taskService.editTaskFormData._id }).subscribe({
          next: (success) => {
            console.log(success);
            if (success.task) {
        ...
        this.taskService.addTask({ ...this.taskform.value, user_id: this.userService.userData._id }).subscribe({
          next: (success) => {
            console.log(success);
            if (success.task) {
              this.taskService.taskFormState = false;
              alert(success.message)
              this.userService.userData.tasks.push(success.task as unknown as Task)
              this.userService.categorizeTask()
              this.taskService.editTaskFormData = new Task()
              this.isformTypeUpdate = false;
            } else {
